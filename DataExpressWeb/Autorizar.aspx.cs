...
namespace DataExpressWeb
{
    public partial class Autorizar : System.Web.UI.Page
    {
            ...
            if (Session["idUser"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    try
                    {
                        ...
                        DB.AsignarParametroProcedimiento("@PERMV", System.Data.DbType.Int16, 1);
                    ...
                    gvFacturas2.DataSourceID = null;
                    gvFacturas2.DataSource = DT;
                    gvFacturas2.DataBind();

                    cantidad = gvFacturas2.Rows.Count;
                    if (cantidad == 0)
                    {
                        NA.Visible = false;
                        AT.Visible = false;
            ...
            if (conf == 1)
            {
                ...
                foreach (GridViewRow row in gvFacturas2.Rows)
                {
                    ...
                    if (chk_Seleccionar.Checked)
                    {
                        ...
                        DB.AsignarParametroEntero("@edo", 2);
        ...
        protected void log(string fol, string ses)
        {
            ...
            if (conf == 1)
            {
                mensaje = "EL USUARIO " + ses + "AUTORIZO LA FACTURA POR VALIDAR CON FOLIO: " + fol;
                lg.guardar_Log(mensaje);
            }
            else
            {
                mensaje = "EL USUARIO " + ses + " RECHAZO FACTURA POR VALIDAR CON FOLIO: " + fol;
                lg.guardar_Log(mensaje);
            }
        }
