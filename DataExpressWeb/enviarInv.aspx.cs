...
{
    public partial class enviarInv : System.Web.UI.Page
    {
        string idfact;
        string pdf;
        string xml;
      ...
      string vigeniaCertificado = "";
        ...
        public void  nombreArchivo(string IDEFAC)
        {
            try
            {

                DB.Conectar();
                DB.CrearComando("select PDFARC from ArchivosInvalidos where IDEFAC=@IDEFAC");
                DB.AsignarParametroCadena("@IDEFAC", IDEFAC);
                ...
                if (DR1.Read())
                {
                    ...
                    pdf = DR1[0].ToString();
                    string[] filenamepdfaux = pdf.Split('/');
                    string nombreF = filenamepdfaux[2];

                    string[] filenamepdf = nombreF.Split('.');
                    filename = filenamepdf[0];
     ...
     protected void Page_Load(object sender, EventArgs e)
        {
            DB.Conectar();
            DB.CrearComando("select servidorSMTP,puertoSMTP,sslSMTP,userSMTP,passSMTP,dirdocusinvalidos,emailEnvio from ParametrosSistema");
            ...
            while (DRSMTP.Read())
            {
                ...
                RutaDOC = DRSMTP[5].ToString();
                emailEnviar = DRSMTP[6].ToString();
            }
            DB.Desconectar();
...
EMISOR.RFCEMI,EMISOR.NOMEMI,facturasInvalidas.noAprobacion,facturasInvalidas.anoAprobacion,facturasInvalidas.email
            ...
            if (DR.Read())
            {
                ...
                pdf = DR[3].ToString(); xml = DR[4].ToString(); emisor = DR[5].ToString(); nombreEmisor = DR[6].ToString(); noAprobacion = DR[7].ToString(); anoAprobacion = DR[8].ToString(); correo = DR[9].ToString();
            }
            DB.Desconectar();
            ...
            DB.Conectar();
            DB.CrearComando(@"SELECT dirtxt,dirdocusinvalidos,dirrespaldo from ParametrosSistema");
            DbDataReader DR1 = DB.EjecutarConsulta();
            DR1.Read();
            dirtxt = DR1[0].ToString();
           // dirpdf = DR1[1].ToString() + @"" + emisor + @"\";
            dirpdf = DR1[1].ToString() + emisor + @"\";
            dirbck = DR1[2].ToString();
            DB.Desconectar();
        ...
        {           
          ...
          "</tr>" +
                   ...
                   "</tr>" +
                        ...
                        try
                        {
                            ...
                            DB.CrearComando((@"insert into LogErrorFacturas
                                ...
                                values 
                                (@detalle,@numeroDocumento)"));
                            DB.AsignarParametroCadena("@detalle", "E-mail enviado: " + correo);
                        ...
                        catch (System.Net.Mail.SmtpException ex)
                        {
                            ...
                            DB.AsignarParametroCadena("@detalle", "E-mail no enviado: " + ex.Message);
                            DB.AsignarParametroCadena("@fecha", System.DateTime.Now.ToString());
                            DB.AsignarParametroCadena("@numeroDocumento", folio + serie);
                            DB.EjecutarConsulta1();
                            DB.Desconectar();
            ...
            catch (Exception ex) 
            {
                ...
                DB.AsignarParametroCadena("@detalle", "error: " + ex.ToString() );
                DB.AsignarParametroCadena("@fecha", System.DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss"));
                DB.AsignarParametroCadena("@numeroDocumento", folio + serie);
                DB.EjecutarConsulta1();
                DB.Desconectar();
